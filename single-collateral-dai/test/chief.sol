...
contract DSChief is DSRoles, DSChiefApprovals {
    ...
    function setOwner(address owner_) public override(DSAuth) {
        owner_;
        revert();
    }

    function setAuthority(DSAuthority authority_) public override(DSAuth) {
        authority_;
        revert();
    }

    function isUserRoot(address who)
        public view override(DSRoles)
        returns (bool)
    {
        return (who == hat);
    }
    function setRootUser(address who, bool enabled) public override(DSRoles) {
        who; enabled;
        revert();
    }
}
