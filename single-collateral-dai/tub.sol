...
contract SaiTub is DSThing, SaiTubEvents {
    ...
    //--Tap-setter------------------------------------------------------
    function turn(SaiTap tap_) public note {
        require(address(tap)  == address(0));
        require(address(tap_) != address(0));
        tap = tap_;
    }
    ...
    function drip() public note {
        ...
        if (tax != RAY) {  // optimised
            ...
            inc = rpow(tax, age);
            _chi = rmul(_chi, inc);
            sai.mint(address(tap), rmul(sub(_chi, _chi_), rum));
        }
    ...
    function wipe(bytes32 cup, uint wad) public note {
        ...
        if (ok && val != 0) gov.move(msg.sender, address(pit), wdiv(owe, uint(val)));
    }
    ...
    function cage(uint fit_, uint jam) public note auth {
        ...
        require(gem.transfer(address(tap), jam));
    }
